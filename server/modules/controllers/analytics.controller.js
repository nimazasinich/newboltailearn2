export class AnalyticsController {
    ...
    async getBasicAnalytics(req, res) {
        try {
      ...
      `).all();
            const summary = {
                totalModels: this.db.prepare('SELECT COUNT(*) as count FROM models').get().count,
                activeTraining: this.db.prepare("SELECT COUNT(*) as count FROM models WHERE status = 'training'").get().count,
                completedModels: this.db.prepare("SELECT COUNT(*) as count FROM models WHERE status = 'completed'").get().count,
                totalDatasets: this.db.prepare('SELECT COUNT(*) as count FROM datasets').get().count
            };
            res.json({
                modelStats,
                trainingStats,
    ...
    async getUsageStats(req, res) {
      ...
      `).all();
            const systemStats = {
                ...
                totalModels: this.db.prepare('SELECT COUNT(*) as count FROM models').get().count,
                totalDatasets: this.db.prepare('SELECT COUNT(*) as count FROM datasets').get().count,
                totalTrainingSessions: this.db.prepare('SELECT COUNT(*) as count FROM training_sessions').get().count
            };
            res.json({
                userStats,
                trainingStats,
