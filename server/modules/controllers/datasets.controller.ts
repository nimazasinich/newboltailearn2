...
export class DatasetsController {
  ...
  async listDatasets(req: Request, res: Response): Promise<void> {
    try {
      ...
      const total = this.db.prepare('SELECT COUNT(*) as count FROM datasets').get() as { count: number };
  ...
  async downloadDataset(req: Request, res: Response): Promise<void> {
    try {
      ...
      const dataset = this.db.prepare('SELECT * FROM datasets WHERE id = ?').get(id) as any;
      if (!dataset) {
        res.status(404).json({ error: 'Dataset not found' });
        return;
      }
