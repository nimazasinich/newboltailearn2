...
function loadDatasetFromDb(db, datasetId) {
    // حالت 1: جدول dataset_items(text,label)
    if (tableExists(db, "dataset_items")) {
        const rows = db.prepare(`SELECT text, label FROM dataset_items WHERE dataset_id = ?`).all(datasetId);
        if (rows?.length) {
            return rows.map((r) => ({ text: String(r.text), label: Number(r.label) }));
        }
    }
