...
import { getHFHeaders, testHFConnection } from './utils/decode.js';
...
defaultDatasets.forEach(dataset => {
    insertDataset.run(dataset.id, dataset.name, dataset.source, dataset.huggingface_id, dataset.samples, dataset.size_mb, dataset.status || 'available', dataset.type || 'text', dataset.description || '');
});
...
app.get('/api/datasets/search', async (req, res) => {
    try {
        ...
        logToDatabase('error', 'huggingface', 'Dataset search failed', { error: error.message });
...
app.get('/api/system/metrics', (req, res) => {
    try {
        ...
        logToDatabase('error', 'system', 'Failed to get system metrics', { error: error.message });
...
app.post('/api/auth/login', async (req, res) => {
    try {
        ...
        logToDatabase('error', 'auth', 'Login failed', { error: error.message });
...
app.post('/api/auth/register', async (req, res) => {
    try {
        ...
        logToDatabase('error', 'auth', 'Registration failed', { error: error.message });
...
app.get('/api/auth/me', requireAuth, (req, res) => {
    try {
        ...
        logToDatabase('error', 'auth', 'Get user info failed', { error: error.message });
...
app.put('/api/auth/profile', requireAuth, async (req, res) => {
    try {
        ...
        logToDatabase('error', 'auth', 'Profile update failed', { error: error.message });
...
app.post('/api/auth/change-password', requireAuth, async (req, res) => {
    try {
        ...
        logToDatabase('error', 'auth', 'Password change failed', { error: error.message });
...
app.get('/api/users', requireAuth, requireRole('admin'), (req, res) => {
    try {
        ...
        logToDatabase('error', 'auth', 'Get users failed', { error: error.message });
...
app.delete('/api/users/:id', requireAuth, requireRole('admin'), (req, res) => {
    try {
        ...
        logToDatabase('error', 'auth', 'Delete user failed', { error: error.message });
        ...
        setTimeout(async () => {
            try {
                ...
                for (let i = 0; i < iterations; i++) {
                    ...
                    const baseScore = model.accuracy || 0.5;
