...
router.get('/', requireAuth, async (req, res) => {
    try {
        ...
        const appError = errorHandler.handleTrainingError(error, 'list_models');
...
router.get('/:modelId', requireAuth, async (req, res) => {
    try {
        ...
        const appError = errorHandler.handleTrainingError(error, 'get_model_metadata', req.params.modelId);
        res.status(appError.statusCode || 500).json({ error: appError.message });
    }
});
/**
 * Load a model
 */
router.post('/:modelId/load', requireAuth, requireRole('trainer'), async (req, res) => {
    try {
        const { modelId } = req.params;
        ...
        const appError = errorHandler.handleTrainingError(error, 'load_model', req.params.modelId);
        res.status(appError.statusCode || 500).json({ error: appError.message });
    }
});
/**
 * Delete a model
 */
router.delete('/:modelId', requireAuth, requireRole('admin'), async (req, res) => {
    try {
        const { modelId } = req.params;
        ...
        const appError = errorHandler.handleTrainingError(error, 'delete_model', req.params.modelId);
        res.status(appError.statusCode || 500).json({ error: appError.message });
    }
});
/**
 ...
 */
router.patch('/:modelId', requireAuth, requireRole('trainer'), async (req, res) => {
    try {
        const { modelId } = req.params;
        ...
        const appError = errorHandler.handleTrainingError(error, 'update_model_metadata', req.params.modelId);
        res.status(appError.statusCode || 500).json({ error: appError.message });
    }
});
/**
 ...
 */
router.head('/:modelId', requireAuth, async (req, res) => {
    try {
        const { modelId } = req.params;
 ...
 * Get model file path
 */
router.get('/:modelId/path', requireAuth, requireRole('trainer'), async (req, res) => {
    try {
        const { modelId } = req.params;
        ...
        const appError = errorHandler.handleTrainingError(error, 'get_model_path', req.params.modelId);
        res.status(appError.statusCode || 500).json({ error: appError.message });
    }
});
/**
 ...
 */
router.get('/:modelId/export', requireAuth, requireRole('trainer'), async (req, res) => {
    try {
        const { modelId } = req.params;
        const modelPath = modelPersistence.getModelPath(modelId);
        if (!modelPath) {
        ...
        const appError = errorHandler.handleTrainingError(error, 'export_model', req.params.modelId);
        res.status(appError.statusCode || 500).json({ error: appError.message });
    }
});
/**
 * Import model (upload)
 */
router.post('/import', requireAuth, requireRole('admin'), upload.single('model'), async (req, res) => {
    try {
        if (!req.file) {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(metadata.id, metadata.name, metadata.type, metadata.status, metadata.accuracy, metadata.loss, metadata.epochs, metadata.current_epoch, metadata.config, metadata.created_at, metadata.updated_at, metadata.created_by);
        res.json({
            success: true,
        ...
        const appError = errorHandler.handleTrainingError(error, 'import_model');
        res.status(appError.statusCode || 500).json({ error: appError.message });
    }
});
/**
...
router.get('/stats/overview', requireAuth, async (req, res) => {
    try {
        ...
        const appError = errorHandler.handleTrainingError(error, 'get_model_stats');
        res.status(appError.statusCode || 500).json({ error: appError.message });
    }
});
