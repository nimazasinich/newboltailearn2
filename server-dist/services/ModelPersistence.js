...
export class ModelPersistence {
    modelsDir;
    db;
    constructor(database, modelsDir = "./models/saved") {
        this.db = database;
     ...
     */
    async saveModel(model, tokenizer, options) {
        try {
      ...
      `).run(modelId, options.modelName, options.modelType, 'completed', options.accuracy, options.loss, options.epochs, options.epochs, // current_epoch = total epochs when completed
            JSON.stringify({
                vocabSize: options.vocabSize,
                maxLen: options.maxLen,
                numClasses: options.numClasses,
                filePath: modelPath,
                tokenizerPath: tokenizerPath
            }), metadata.createdAt, metadata.updatedAt, options.createdBy);
    ...
    updateModelMetadata(modelId, updates) {
        try {
            const fields = Object.keys(updates).filter(key => ['name', 'accuracy', 'loss', 'epochs'].includes(key));
            if (fields.length === 0) {
                return true;
            }
...
//# sourceMappingURL=ModelPersistence.js.map