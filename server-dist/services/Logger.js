...
export class Logger {
    ...
    async writeToDatabase(entry) {
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
      `).run(entry.level, entry.message, entry.category, entry.metadata ? JSON.stringify(entry.metadata) : null, entry.userId, entry.requestId, entry.source, entry.timestamp);
        }
        catch (error) {
            console.error('Failed to write to database:', error);
        }
    }
    async log(level, message, category = 'general', metadata, userId, requestId, source) {
    ...
    async debug(message, category = 'general', metadata, userId, requestId, source) {
        await this.log('debug', message, category, metadata, userId, requestId, source);
    }
    async info(message, category = 'general', metadata, userId, requestId, source) {
        await this.log('info', message, category, metadata, userId, requestId, source);
    }
    async warn(message, category = 'general', metadata, userId, requestId, source) {
        await this.log('warn', message, category, metadata, userId, requestId, source);
    }
    async error(message, category = 'general', metadata, userId, requestId, source) {
    ...
    async logDatabase(operation, table, duration, error) {
        const metadata = {
            operation,
            table,
            duration,
    ...
    async logTraining(operation, modelId, progress, error) {
        const metadata = {
            operation,
            modelId,
            progress,
    ...
    async logAuth(action, userId, success = true, error) {
        const metadata = {
            action,
            success,
            error: error?.message
    ...
    getLogs(level, category, userId, startDate, endDate, limit = 100, offset = 0) {
        if (!this.config.database)
            return [];
        try {
            ...
            const logs = this.config.database.prepare(query).all(...params);
            return logs.map(log => ({
                id: log.id,
                level: log.level,
                message: log.message,
