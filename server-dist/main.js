    ...
    setupRoutes() {
        ...
        this.app.post('/api/documents/process', async (req, res) => {
            try {
                ...
                const appError = this.errorHandler.handleFileSystemError(error, 'process_document');
        ...
        this.app.post('/api/training/start', async (req, res) => {
            try {
                ...
                const appError = this.errorHandler.handleTrainingError(error, 'start_training');
                res.status(appError.statusCode || 500).json({ error: appError.message });
            }
        });
        // SPA fallback
