...
export function createModelsRoutes(modelsController) {
    ...
    router.post('/', requireAuth, requireRole('trainer'), csrfProtection.middleware(), validate(schemas.createModel), modelsController.createModel.bind(modelsController));
    // Update model (trainer/admin only)
    router.put('/:id', requireAuth, requireRole('trainer'), csrfProtection.middleware(), validate(schemas.updateModel), modelsController.updateModel.bind(modelsController));
    // Delete model (admin only)
    router.delete('/:id', requireAuth, requireRole('admin'), csrfProtection.middleware(), modelsController.deleteModel.bind(modelsController));
    // Get model logs
    ...
    // Export model
    router.post('/:id/export', requireAuth, requireRole('trainer'), csrfProtection.middleware(), modelsController.exportModel.bind(modelsController));
    return router;
}
