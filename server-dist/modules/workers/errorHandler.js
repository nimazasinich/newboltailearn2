...
export class WorkerErrorHandler {
     ...
     */
    handleMainThreadError(error) {
        const mainThreadError = {
     ...
     */
    async attemptRecovery(worker, workerError) {
        ...
        if (strategy.exponentialBackoff) {
            delay = Math.min(strategy.retryDelay * Math.pow(2, workerError.recoveryAttempts - 1), strategy.maxBackoffDelay);
        }
    ...
    logError(workerError) {
      ...
      `).run(workerError.status === 'failed' ? 'error' : 'warning', `Worker ${workerError.workerId} error: ${workerError.error}`, JSON.stringify(workerError));
        }
        catch (error) {
     ...
     */
    getWorkerHealthStatus(workerId) {
     ...
     */
    setRecoveryStrategy(workerId, strategy) {
        this.recoveryStrategies.set(workerId, strategy);
    }
    /**
     * Get error history
     */
    getErrorHistory(hours = 24) {
        try {
