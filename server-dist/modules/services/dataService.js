import { useFakeData, isDemoMode } from '../security/config';
/**
...
export class DataService {
    ...
    createDataset(data) {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(data.id || `dataset-${Date.now()}`, data.name, data.source, data.huggingface_id, data.samples || 0, data.size_mb || 0, data.status || 'available', data.type || 'text', data.description || '');
        return {
    ...
    updateDataset(id, updates) {
        ...
        const fields = Object.keys(updates).filter(key => ['name', 'status', 'samples', 'size_mb'].includes(key));
        if (fields.length === 0) {
            return null;
        }
     ...
     */
    getTrainingLogs(modelId, limit = 50) {
        if (this.useFakeData) {
     ...
     */
    getFakeTrainingLogs(modelId, limit) {
        const logs = [];
