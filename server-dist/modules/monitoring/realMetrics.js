 ...
 */
export function setupRealMetrics(app) {
 ...
 */
export function recordTrainingStart(modelType) {
 ...
 */
export function recordTrainingEnd() {
    trainingSessionsActive.dec();
}
/**
 * Update model metrics
 */
export function updateModelMetrics(modelId, modelType, accuracy, loss) {
 ...
 */
export function recordDatabaseQuery(operation, table, duration) {
    databaseQueryDuration.observe({ operation, table }, duration / 1000);
}
/**
 * Record authentication attempt
 */
export function recordAuthAttempt(status, method) {
    authenticationAttempts.inc({ status, method });
}
/**
 * Record application error
 */
export function recordError(type, severity) {
    errorRate.inc({ type, severity });
}
// Export metrics for testing
export { register as metricsRegistry };
//# sourceMappingURL=realMetrics.js.map