#!/bin/bash
# generate-report-simple.sh - Generate deployment artifacts and reports (no jq required)
...
# Generate BUILD.json (simple version)
echo -e "${GREEN}Generating BUILD.json...${NC}"
cat > "$ARTIFACTS_DIR/BUILD.json" << EOF
{
  ...
  "status": "ready",
  "message": "Docker build configuration complete"
}
EOF

# Generate COMPOSE.json (simple version)
echo -e "${GREEN}Generating COMPOSE.json...${NC}"
cat > "$ARTIFACTS_DIR/COMPOSE.json" << EOF
{
  "timestamp": "$(date -Iseconds)",
  "profiles": ["dev", "prod", "nginx", "gpu"],
  "services": {
    "backend": {
      "port": 3000,
      "health": "/health",
...
# Check if DB_SMOKE.json exists
if [ ! -f "$ARTIFACTS_DIR/DB_SMOKE.json" ]; then
    echo -e "${YELLOW}DB_SMOKE.json not found. Creating placeholder...${NC}"
    cat > "$ARTIFACTS_DIR/DB_SMOKE.json" << EOF
{
  "timestamp": "$(date -Iseconds)",
  "status": "not_run",
  "message": "Database smoke test has not been executed yet",
  "instructions": "Run: docker exec persian-legal-backend /app/docker/scripts/db-smoke-test.sh"
}
EOF
fi

# Generate the full deployment report
echo -e "${GREEN}Generating DEPLOYMENT.md...${NC}"
bash -c 'cat > "$REPORTS_DIR/DEPLOYMENT.md"' << 'REPORT_EOF'
...
---
*Report generated automatically by generate-report-simple.sh*
REPORT_EOF

# Replace timestamp placeholder
sed -i "s/TIMESTAMP_PLACEHOLDER/$(date)/" "$REPORTS_DIR/DEPLOYMENT.md"
...
ls -la "$ARTIFACTS_DIR"/*.json 2>/dev/null || echo "No JSON artifacts yet"
ls -la "$REPORTS_DIR"/*.md 2>/dev/null || echo "No reports yet"